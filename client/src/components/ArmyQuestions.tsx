...
import { Stack, FormControl, FormControlLabel, FormLabel, Radio, RadioGroup, TextField, Typography, MenuItem } from "@mui/material";
...
export const ArmyQuestions: React.FC<{
  ...
  useEffect(() => {
    if (enlistDate && releaseDate && releaseDate.isAfter(enlistDate)) {
      const today = dayjs();
      if (today.diff(enlistDate, "month") < 24) { setServiceType("mandatory"); }
  ...
  return (
    ...
    >
      <Typography variant="body1" gutterBottom>
        בחלק זה תתבקש למלא פרטים אודות שירותך הצבאי.
