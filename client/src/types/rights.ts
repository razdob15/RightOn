...
import { SoldierType, HousingStatus, UserStatus, ServiceType } from './user-status';
import { differenceInDays, differenceInMonths } from 'date-fns';
import { useReducer } from 'react';

const hasHouse = (userStatus: UserStatus) => [HousingStatus.OWNS, HousingStatus.RENTS].includes(userStatus.housing.housingStatus)
...
export const rightsData: Right[] = [
  ...
  },
  {
    ...
    details: "Lone soldiers are allowed to stay permanently in one of the 'Association for the Well-being of Soldiers' (Beit HaHayal) facilities.\nAt Beit HaHayal, soldiers will be provided with laundry and food services.\nEligibility for 3 months of accommodation after release.",
    sourceOrganization: "Kol Zchut",
    ...
    isEligible: (userStatus: UserStatus) => {
      if (userStatus.housing.housingStatus === HousingStatus.NO_HOUSE) {
        ...
        if (userStatus.service.dutyEndDate && differenceInMonths(userStatus.service.dutyEndDate, new Date()) <= 3) return true;
      }
      return false;
    },
  },
  {
    ...
    details: "During regular leave, long weekend leave, and holidays, lone soldiers are entitled to free accommodation in any of the Beit HaHayal facilities located throughout the country.\nDuring leave days, the soldier will receive free meals through Beit HaHayal.\nIn cases where Beit HaHayal does not provide meals, the soldier will be entitled to a per diem allowance through the 'Association for the Well-being of Soldiers'.",
    sourceOrganization: "Kol Zchut",
    grantingOrganization: "IDF and the Association for the Well-being of Soldiers",
    eligibleSoldierType: [SoldierType.DISTINGUISHED_LONE_SOLDIER, SoldierType.LONE_SOLDIER],
    isEligible: (userStatus: UserStatus) => userStatus.service.serviceType === ServiceType.MANDATORY
  },
  {
    rightName: "Accommodation in Kibbutzim for Lone Soldiers",
    details: "The kibbutzim provide soldiers with a furnished room, kitchenette, shower and toilet, laundry and food services. In addition, there are 'adoptive families' for lone soldiers in the kibbutzim.\nA lone soldier living in a kibbutz receives a kibbutz grant of NIS 150 per month.\nCombat soldiers are entitled to stay in a separate room in the kibbutz.",
    sourceOrganization: "Kol Zchut",
    grantingOrganization: "IDF",
    eligibleSoldierType: [SoldierType.DISTINGUISHED_LONE_SOLDIER, SoldierType.LONE_SOLDIER],
    isEligible: (userStatus: UserStatus) => userStatus.housing.housingStatus === HousingStatus.NO_HOUSE && userStatus.service.serviceType ===
      ServiceType.MANDATORY

  },
  {
    rightName: "ALACH Apartments for Lone Soldiers",
    details: "The apartments, located in various communities throughout the country, are furnished and equipped so that soldiers can maintain a proper lifestyle during their military service.\nMost are four-room apartments intended for six soldiers, so that every two soldiers share a room.\nThe soldiers are not required to pay rent or taxes for the apartment, and the Association for the Well-being of Soldiers provides them with laundry and food services.",
    sourceOrganization: "Kol Zchut",
    grantingOrganization: "IDF and the Association for the Well-being of Soldiers",
    eligibleSoldierType: [SoldierType.DISTINGUISHED_LONE_SOLDIER, SoldierType.LONE_SOLDIER],
    isEligible: (userStatus: UserStatus) => userStatus.service.serviceType === ServiceType.MANDATORY
  },
  {
    ...
    details: "The discount given each month is 50% for the first 400 kilowatt-hours each month (800 kWh in a bi-monthly bill) at the residential rate, in the home used for the beneficiaries' residence.",
    sourceOrganization: "Kol Zchut",
  ...
  },
  {
    ...
    details: "Soldiers in mandatory service are entitled to an exemption from paying municipal tax (Arnona) during their military service, and for up to 4 months from the date of their release from mandatory service.\nThe exemption from paying Arnona is given for 70 square meters of the apartment area.\nIn apartments where 5 or more people live (more than 4 people), an exemption is given for 90 square meters of the apartment area.",
    sourceOrganization: "Kol Zchut",
    grantingOrganization: "Local Authority",
    eligibleSoldierType: [SoldierType.DISTINGUISHED_LONE_SOLDIER, SoldierType.LONE_SOLDIER],
    isEligible: (userStatus: UserStatus) => {
      if (hasHouse(userStatus)) {
        if (userStatus.service.serviceType === ServiceType.MANDATORY || userStatus.service.dutyEndDate && differenceInMonths(userStatus.service.dutyEndDate, new Date()) <= 4) return true;
      }
